...
class SendEventNotificationsService @Inject()(appConfig: AppConfig,
                                              ...
                                              clock: Clock) extends ScheduledService[Either[ScheduleStatus.JobFailed, Boolean]] with ApplicationLogger {
  ...
  def tryLock(f: => Future[Either[ScheduleStatus.JobFailed, Boolean]])
             (implicit ec: ExecutionContext): Future[Either[ScheduleStatus.JobFailed, Boolean]] = {
